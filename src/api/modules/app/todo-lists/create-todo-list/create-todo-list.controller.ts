...
export class CreateTodoListController {
  ...
  async createTodoList(@Body() body: CreateTodoListRequestDto): Promise<{ id: string }> {
    const result = await this.commandBus.execute(new CreateTodoListCommand(body.title, body.colourCode));
